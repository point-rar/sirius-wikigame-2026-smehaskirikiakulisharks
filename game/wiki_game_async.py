                ...
                try:
                    ...
                    try:
                        ...
                        if direction == 0: # Forward
                            logger.debug(f"Forward: Parsing '{cur_page.page_name}' (Depth {cur_page.depth}, Prio {-prio})")
                            links_method = self.wiki_parser.get_links
                            own_visited = visited_forward
                            other_visited = visited_backward
                        else: # Backward
                            logger.debug(f"Backward: Parsing backlinks for '{cur_page.page_name}' (Depth {cur_page.depth}, Prio {-prio})")
                        ...
                        for link in links:
                            ...
                            if next_page_name in other_visited:
                                ...
                                if direction == 0:
                                    path_f = Page(next_page_name, cur_page.depth + 1, cur_page).path_to_root().page_names
                                    path_b = meeting_page_other.path_to_root().page_names
                                else:
                                    path_b = Page(next_page_name, cur_page.depth + 1, cur_page).path_to_root().page_names
                        ...
                        if candidates and not stop_event.is_set():
                            ...
                            for next_page in candidates:
                                ...
                                await add_to_queue(-length, next_page.page_name, next_page, direction)
